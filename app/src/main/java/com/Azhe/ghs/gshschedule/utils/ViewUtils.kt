...
object ViewUtils {
    ...
    fun getViewBitmap(viewGroup: ViewGroup, low: Boolean = false, marginBottom: Int = 0): Bitmap {
        ...
        for (i in 0 until viewGroup.childCount) {
            ...
            val childH = if (child.measuredHeight > 0) child.measuredHeight else child.height
            h += childH + marginBottom
        }
        val w = if (viewGroup.measuredWidth > 0) viewGroup.measuredWidth else viewGroup.width
        // 创建对应大小的bitmap
        val bitmapW = if (w > 0) w else 1
        val bitmapH = if (h > 0) h else 1
        val bitmap = Bitmap.createBitmap(bitmapW, bitmapH,
    ...
    fun layoutView(v: View, width: Int, height: Int) {
        // measure first, then layout — correct order
